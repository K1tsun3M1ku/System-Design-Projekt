...
#define RC_PROP_SLEEP_TIMEOUT                                                  \
  0x2 /*!< Set/get the NXT sleep timeout value (times 60000) */
#define RC_PROP_DEBUGGING                                                      \
  0xF     /*!< Set/get enhanced firmware debugging information */
...
#define OPARR_SUM                                                              \
  0x00 /*!< Calculate the sum of the elements in the numeric input array */
#define OPARR_MEAN                                                             \
...
#define RADIANS_PER_DEGREE                                                     \
  PI / 180 /*!< Used for converting from degrees to radians */
#define DEGREES_PER_RADIAN                                                     \
  180 / PI /*!< Used for converting from radians to degrees */
#endif
/** @} */ // end of MiscConstants group
...
#define InputIOInputMode(p)                                                    \
  (IO_BASE + MOD_INPUT + InputModeField + ((p) * IO_IN_FPP))
#define InputIORawValue(p)                                                     \
  (IO_BASE + MOD_INPUT + RawValueField + ((p) * IO_IN_FPP))
#define InputIONormalizedValue(p)                                              \
  (IO_BASE + MOD_INPUT + NormalizedValueField + ((p) * IO_IN_FPP))
#define InputIOScaledValue(p)                                                  \
  (IO_BASE + MOD_INPUT + ScaledValueField + ((p) * IO_IN_FPP))
#define InputIOInvalidData(p)                                                  \
  (IO_BASE + MOD_INPUT + InvalidDataField + ((p) * IO_IN_FPP))

#define OutputIOUpdateFlags(p)                                                 \
  (IO_BASE + MOD_OUTPUT + UpdateFlagsField + ((p) * IO_OUT_FPP))
#define OutputIOOutputMode(p)                                                  \
  (IO_BASE + MOD_OUTPUT + OutputModeField + ((p) * IO_OUT_FPP))
#define OutputIOPower(p)                                                       \
  (IO_BASE + MOD_OUTPUT + PowerField + ((p) * IO_OUT_FPP))
#define OutputIOActualSpeed(p)                                                 \
  (IO_BASE + MOD_OUTPUT + ActualSpeedField + ((p) * IO_OUT_FPP))
#define OutputIOTachoCount(p)                                                  \
  (IO_BASE + MOD_OUTPUT + TachoCountField + ((p) * IO_OUT_FPP))
#define OutputIOTachoLimit(p)                                                  \
  (IO_BASE + MOD_OUTPUT + TachoLimitField + ((p) * IO_OUT_FPP))
#define OutputIORunState(p)                                                    \
  (IO_BASE + MOD_OUTPUT + RunStateField + ((p) * IO_OUT_FPP))
#define OutputIOTurnRatio(p)                                                   \
  (IO_BASE + MOD_OUTPUT + TurnRatioField + ((p) * IO_OUT_FPP))
#define OutputIORegMode(p)                                                     \
  (IO_BASE + MOD_OUTPUT + RegModeField + ((p) * IO_OUT_FPP))
#define OutputIOOverload(p)                                                    \
  (IO_BASE + MOD_OUTPUT + OverloadField + ((p) * IO_OUT_FPP))
#define OutputIORegPValue(p)                                                   \
  (IO_BASE + MOD_OUTPUT + RegPValueField + ((p) * IO_OUT_FPP))
#define OutputIORegIValue(p)                                                   \
  (IO_BASE + MOD_OUTPUT + RegIValueField + ((p) * IO_OUT_FPP))
#define OutputIORegDValue(p)                                                   \
  (IO_BASE + MOD_OUTPUT + RegDValueField + ((p) * IO_OUT_FPP))
#define OutputIOBlockTachoCount(p)                                             \
  (IO_BASE + MOD_OUTPUT + BlockTachoCountField + ((p) * IO_OUT_FPP))
#define OutputIORotationCount(p)                                               \
  (IO_BASE + MOD_OUTPUT + RotationCountField + ((p) * IO_OUT_FPP))
...
#define FileOpenAppend                                                         \
  2                 /*!< Open a file for appending to the end of the file */
...
#define FileResolveHandle                                                      \
  6 /*!< Get a file handle for the specified filename if it is already open */
#define FileRename 7    /*!< Rename a file */
#define FileDelete 8    /*!< Delete a file */
#define SoundPlayFile 9 /*!< Play a sound or melody file */
#define SoundPlayTone                                                          \
  10 /*!< Play a simple tone with the specified frequency and duration */
...
#define CommLSCheckStatus                                                      \
  23                    /*!< Check the status of a lowspeed (aka I2C) device */
...
#define CommBTConnection                                                       \
  36                   /*!< Connect or disconnect to a known bluetooth device */
...
#define UpdateCalibCacheInfo                                                   \
  43                       /*!< Update sensor calibration cache information */
...
#define ListFiles                                                              \
  47 /*!< List files that match the specified filename pattern */
...
#define IOMapWriteByID                                                         \
  79 /*!< Write data to one of the firmware module's IOMap structures using    \
        the module's ID */
#define DisplayExecuteFunction                                                 \
  80 /*!< Execute one of the Display module's internal functions */
#define CommExecuteFunction                                                    \
  81 /*!< Execute one of the Comm module's internal functions */
#define LoaderExecuteFunction                                                  \
  82 /*!< Execute one of the Loader module's internal functions */
#define FileFindFirst                                                          \
  83 /*!< Start a search for a file using a filename pattern */
...
#define DrawGraphicArray                                                       \
  92 /*!< Draw a graphic image from a byte array to the LCD screen */
#define DrawPolygon 93 /*!< Draw a polygon on the LCD screen */
#define DrawEllipse 94 /*!< Draw an ellipse on the LCD screen */
#define DrawFont                                                               \
  95 /*!< Draw text using a custom RIC-based font to the LCD screen */
...
#define STAT_MSG_EMPTY_MAILBOX                                                 \
  64                         /*!< Specified mailbox contains no new messages */
...
#define ERR_CLUMP_COUNT                                                        \
  -7 /*!< 0xF9 (FileClumpCount == 0 || FileClumpCount >= NOT_A_CLUMP) */
#define ERR_NO_CODE -8 /*!< 0xF8 VarsCmd.CodespaceCount == 0 */
#define ERR_INSANE_OFFSET                                                      \
  -9 /*!< 0xF7 CurrOffset != (DataSize - VarsCmd.CodespaceCount * 2) */
...
#define ERR_LOADER_ERR                                                         \
  -11 /*!< 0xF5 LOADER_ERR(LStatus) != SUCCESS || pData == NULL || DataSize == \
         0 */
#define ERR_SPOTCHECK_FAIL                                                     \
  -12 /*!< 0xF4 ((UBYTE*)(VarsCmd.pCodespace) < pData) (c_cmd.c 1893) */
...
#define ERR_INVALID_FIELD                                                      \
  -17 /*!< 0xEF Attempted to access invalid field of a structure */
 ...
 * @{
 */
#define ERR_COMM_CHAN_NOT_READY                                                \
  -32 /*!< 0xE0 Specified channel/connection not configured or busy */
#define ERR_COMM_CHAN_INVALID                                                  \
  -33 /*!< 0xDF Specified channel/connection is not valid */
#define ERR_COMM_BUFFER_FULL -34 /*!< 0xDE No room in comm buffer */
#define ERR_COMM_BUS_ERR                                                       \
  -35     /*!< 0xDD Something went wrong on the communications bus */
...
#define ERR_RC_FAILED                                                          \
  -67     /*!< 0xBD Request failed (i.e. specified file not found) */
...
#define CommandOffsetPRCHandler                                                \
  16                         /*!< Offset to the RC Handler function pointer */
#define CommandOffsetTick 20 /*!< Offset to the VM's current tick */
#define CommandOffsetOffsetDS                                                  \
  24 /*!< Offset to the running program's data space (DS) */
#define CommandOffsetOffsetDVA                                                 \
  26 /*!< Offset to the running program's DOPE vector address (DVA) */
#define CommandOffsetProgStatus                                                \
  28                          /*!< Offset to the running program's status */
...
#define CommandOffsetFileName                                                  \
  32 /*!< Offset to the running program's filename */
...
#define LoaderOffsetPFunc                                                      \
  0 /*!< Offset to the Loader module function pointer                          \
     */
#define LoaderOffsetFreeUserFlash                                              \
  4       /*!< Offset to the amount of free user flash */
...
#define LDR_INPROGRESS                                                         \
  0x0001 /*!< The function is executing but has not yet completed. */
...
#define LDR_NOMOREHANDLES                                                      \
  0x8100 /*!< All available file handles are in use.                           \
          */
#define LDR_NOSPACE                                                            \
  0x8200 /*!< Not enough free flash memory for the specified file size. */
#define LDR_NOMOREFILES                                                        \
  0x8300 /*!< The maximum number of files has been reached. */
...
#define LDR_HANDLEALREADYCLOSED                                                \
  0x8800 /*!< The file handle has already been closed. */
#define LDR_NOLINEARSPACE                                                      \
  0x8900 /*!< Not enough linear flash memory is available. */
...
#define LDR_FILEEXISTS                                                         \
  0x8F00 /*!< A file with the same name already exists.                        \
          */
#define LDR_MODULENOTFOUND                                                     \
  0x9000 /*!< No modules matched the specified search criteria. */
#define LDR_OUTOFBOUNDARY                                                      \
  0x9100 /*!< Specified IOMap offset is outside the bounds of the IOMap. */
...
#define LDR_BTTIMEOUT                                                          \
  0x9600 /*!< A timeout in the bluetooth system has occurred. */
#define LDR_FILETX_TIMEOUT                                                     \
  0x9700 /*!< Error transmitting file: a timeout occurred. */
#define LDR_FILETX_DSTEXISTS                                                   \
  0x9800 /*!< Error transmitting file: destination file exists. */
#define LDR_FILETX_SRCMISSING                                                  \
  0x9900 /*!< Error transmitting file: source file is missing. */
#define LDR_FILETX_STREAMERROR                                                 \
  0x9A00 /*!< Error transmitting file: a stream error occurred. */
#define LDR_FILETX_CLOSEERROR                                                  \
  0x9B00 /*!< Error transmitting file: attempt to close file failed. */
...
#define LDR_CMD_FINDFIRST                                                      \
  0x86 /*!< Find the first file matching the specified pattern */
#define LDR_CMD_FINDNEXT                                                       \
  0x87 /*!< Find the next file matching the specified pattern */
...
#endif
#define LDR_CMD_FINDFIRSTMODULE                                                \
  0x90 /*!< Find the first module matching the specified pattern */
#define LDR_CMD_FINDNEXTMODULE                                                 \
  0x91 /*!< Find the next module matching the specified pattern */
...
#define LDR_CMD_DELETEUSERFLASH                                                \
  0xA0                          /*!< Delete all files from user flash memory */
...
#define LDR_CMD_BTFACTORYRESET                                                 \
  0xA4 /*!< Reset bluetooth configuration to factory defaults */
#if defined(__ENHANCED_FIRMWARE) && (__FIRMWARE_VERSION > 107)
...
#define SOUND_FLAGS_RUNNING                                                    \
  0x02    /*!< R  - Currently processing a tone or file */
 ...
 * @{
 */
#define SOUND_STATE_IDLE                                                       \
  0x00 /*!< R  - Idle, ready for start sound (SOUND_UPDATE) */
#define SOUND_STATE_FILE                                                       \
  0x02 /*!< R  - Processing a file of sound/melody data */
#define SOUND_STATE_TONE 0x03 /*!< R  - Processing a play tone request */
#define SOUND_STATE_STOP                                                       \
  0x04    /*!< W  - Stop sound immediately and close hardware */
...
#define SOUND_MODE_LOOP                                                        \
  0x01 /*!< W  - Play file until writing SOUND_STATE_STOP into SoundState */
#define SOUND_MODE_TONE                                                        \
  0x02    /*!< W  - Play tone specified in Frequency for Duration ms */
...
#define SoundOffsetSampleRate                                                  \
  4 /*!< RW - Sound file sample rate [2000..16000] (2 bytes) */
...
#define SoundOffsetMode                                                        \
  28 /*!< RW - Play mode  - described above (1 byte) \ref SoundModeConstants   \
      */
#define SoundOffsetVolume                                                      \
  29      /*!< RW - Sound/melody volume [0..4] 0 = off (1 byte) */
...
#define BTNSTATE_SHORT_RELEASED_EV                                             \
  0x02 /*!< Button is in the short released state. */
#define BTNSTATE_LONG_PRESSED_EV                                               \
  0x04 /*!< Button is in the long pressed state. */
#define BTNSTATE_LONG_RELEASED_EV                                              \
  0x08 /*!< Button is in the long released state. */
#define BTNSTATE_PRESSED_STATE                                                 \
  0x80                     /*!< A bitmask for the button pressed state */
...
#define ButtonOffsetLongRelCnt(b)                                              \
  (((b) * 8) + 3) /*!< Offset to the LongRelCnt field. This field stores the   \
                     long release count. */
#define ButtonOffsetRelCnt(b)                                                  \
  (((b) * 8) +                                                                 \
   4) /*!< Offset to the RelCnt field. This field stores the release count. */
...
#define UI_FLAGS_DISABLE_LEFT_RIGHT_ENTER                                      \
  0x02 /*!< RW - Disable left, right and enter button */
...
#define UI_FLAGS_EXECUTE_LMS_FILE                                              \
  0x20 /*!< W  - Execute LMS file in "LMSfilename" (Try It) */
#define UI_FLAGS_BUSY                                                          \
  0x40 /*!< R  - UI busy running or datalogging (popup disabled) */
#define UI_FLAGS_ENABLE_STATUS_UPDATE                                          \
  0x80    /*!< W  - Enable status line to be updated */
 ...
 * @{
 */
#define UI_STATE_INIT_DISPLAY                                                  \
  0 /*!< RW - Init display and load font, menu etc. */
...
#define UI_STATE_LEFT_PRESSED                                                  \
  8 /*!< RW - Load selected function and next menu id */
#define UI_STATE_RIGHT_PRESSED                                                 \
  9 /*!< RW - Load selected function and next menu id */
#define UI_STATE_ENTER_PRESSED                                                 \
  10 /*!< RW - Load selected function and next menu id */
#define UI_STATE_EXIT_PRESSED                                                  \
  11 /*!< RW - Load selected function and next menu id */
...
#define UI_BT_CONNECT_REQUEST                                                  \
  0x40                         /*!< RW - BT get connect accept in progress */
...
#define UI_VM_RUN_FREE                                                         \
  1 /*!< VM_RUN_FREE: Attempt to run as many instructions as possible within   \
       our timeslice */
#define UI_VM_RUN_SINGLE                                                       \
  2 /*!< VM_RUN_SINGLE: Run exactly one instruction per timeslice */
...
#define UIOffsetBatteryVoltage                                                 \
  4 /*!< R  - Battery voltage in millivolts (2 bytes) */
#define UIOffsetLMSfilename                                                    \
  6 /*!< W  - LMS filename to execute (Try It) (20 bytes) */
#define UIOffsetFlags                                                          \
  26 /*!< RW - Update command flags  (flags enumerated above) (1 byte) */
#define UIOffsetState                                                          \
  27 /*!< RW - UI state              (states enumerated above) (1 byte) */
#define UIOffsetButton                                                         \
  28 /*!< RW - Insert button         (buttons enumerated above) (1 byte) */
#define UIOffsetRunState                                                       \
  29 /*!< W  - VM Run state          (0 = stopped, 1 = running) (1 byte) */
#define UIOffsetBatteryState                                                   \
  30 /*!< W  - Battery state         (0..4 capacity) (1 byte) */
...
#define UIOffsetUsbState                                                       \
  32 /*!< W  - Usb state             (0=disconnected, 1=connected, 2=working)  \
        (1 byte) */
#define UIOffsetSleepTimeout                                                   \
  33 /*!< RW - Sleep timeout time    (min) (1 byte) */
#define UIOffsetSleepTimer                                                     \
  34 /*!< RW - Sleep timer           (min) (1 byte)                            \
      */
#define UIOffsetRechargeable                                                   \
  35 /*!< R  - Rechargeable battery  (0 = no, 1 = yes) (1 byte) */
#define UIOffsetVolume 36 /*!< RW - Volume used in UI     (0 - 4) (1 byte) */
#define UIOffsetError 37  /*!< W  - Error code (1 byte) */
#define UIOffsetOBPPointer                                                     \
  38 /*!< W  - Actual OBP step       (0 - 4) (1 byte) */
#define UIOffsetForceOff                                                       \
  39 /*!< W  - Force off             (> 0 = off) (1 byte) */
...
#define IN_MODE_TRANSITIONCNT                                                  \
  0x40 /*!< Counts the number of boolean transitions */
...
#define IN_MODE_CELSIUS                                                        \
  0xA0 /*!< RCX temperature sensor value in degrees celcius */
#define IN_MODE_FAHRENHEIT                                                     \
  0xC0 /*!< RCX temperature sensor value in degrees fahrenheit */
#define IN_MODE_ANGLESTEP                                                      \
  0xE0 /*!< RCX rotation sensor (16 ticks per revolution) */
 ...
 */
#define TypeField                                                              \
  0 /*!< Type field. Contains one of the sensor type constants. Read/write. */
...
#define INPUT_RED                                                              \
  0 /*!< Access the red value from color sensor value arrays                   \
     */
#define INPUT_GREEN                                                            \
  1 /*!< Access the green value from color sensor value arrays */
#define INPUT_BLUE                                                             \
  2 /*!< Access the blue value from color sensor value arrays */
#define INPUT_BLANK                                                            \
  3 /*!< Access the blank value from color sensor value arrays */
#define INPUT_NO_OF_COLORS                                                     \
  4       /*!< The number of entries in the color sensor value arrays */
...
/** @defgroup InputColorCalibrationStateConstants Color calibration state
 * constants Constants for use with the color calibration state function.
 * \sa ColorCalibrationState()
 * @{
 */
#define INPUT_SENSORCAL                                                        \
  0x01 /*!< The state returned while the color sensor is calibrating */
#define INPUT_SENSOROFF                                                        \
  0x02 /*!< The state returned once calibration has completed */
...
#define InputOffsetCustomZeroOffset(p)                                         \
  (((p) * 20) +                                                                \
   0) /*!< Read/write the zero offset of a custom sensor (2 bytes) uword */
#define InputOffsetADRaw(p)                                                    \
  (((p) * 20) + 2) /*!< Read the AD raw sensor value (2 bytes) uword */
#define InputOffsetSensorRaw(p)                                                \
  (((p) * 20) + 4) /*!< Read the raw sensor value (2 bytes) uword */
#define InputOffsetSensorValue(p)                                              \
  (((p) * 20) + 6) /*!< Read/write the scaled sensor value (2 bytes) sword */
#define InputOffsetSensorType(p)                                               \
  (((p) * 20) + 8) /*!< Read/write the sensor type */
#define InputOffsetSensorMode(p)                                               \
  (((p) * 20) + 9) /*!< Read/write the sensor mode */
#define InputOffsetSensorBoolean(p)                                            \
  (((p) * 20) + 10) /*!< Read the sensor boolean value */
#define InputOffsetDigiPinsDir(p)                                              \
  (((p) * 20) + 11) /*!< Read/write the direction of the Digital pins (1 is    \
                       output, 0 is input) */
#define InputOffsetDigiPinsIn(p)                                               \
  (((p) * 20) + 12) /*!< Read/write the status of the digital pins */
#define InputOffsetDigiPinsOut(p)                                              \
  (((p) * 20) + 13) /*!< Read/write the output level of the digital pins */
#define InputOffsetCustomPctFullScale(p)                                       \
  (((p) * 20) + 14) /*!< Read/write the Pct full scale of the custom sensor */
#define InputOffsetCustomActiveStatus(p)                                       \
  (((p) * 20) +                                                                \
   15) /*!< Read/write the active or inactive state of the custom sensor */
#define InputOffsetInvalidData(p)                                              \
  (((p) * 20) + 16) /*!< Indicates whether data is invalid (1) or valid (0) */

#if __FIRMWARE_VERSION > 107
#define InputOffsetColorCalibration(p, np, nc)                                 \
  (80 + ((p) * 84) + 0 + ((np) * 16) +                                         \
   ((nc) * 4)) /*!< Read/write color calibration point values */
#define InputOffsetColorCalLimits(p, np)                                       \
  (80 + ((p) * 84) + 48 +                                                      \
   ((np) * 2)) /*!< Read/write color calibration limits */
#define InputOffsetColorADRaw(p, nc)                                           \
  (80 + ((p) * 84) + 52 + ((nc) * 2)) /*!< Read AD raw color sensor values */
#define InputOffsetColorSensorRaw(p, nc)                                       \
  (80 + ((p) * 84) + 60 + ((nc) * 2)) /*!< Read raw color sensor values */
#define InputOffsetColorSensorValue(p, nc)                                     \
  (80 + ((p) * 84) + 68 + ((nc) * 2)) /*!< Read scaled color sensor values */
#define InputOffsetColorBoolean(p, nc)                                         \
  (80 + ((p) * 84) + 76 + ((nc) * 2)) /*!< Read color sensor boolean values */
#define InputOffsetColorCalibrationState(p)                                    \
  (80 + ((p) * 84) + 80) /*!< Read color sensor calibration state */
#endif
 ...
 * @{
 */
#define UF_UPDATE_MODE                                                         \
  0x01 /*!< Commits changes to the \ref OutputModeField output property */
#define UF_UPDATE_SPEED                                                        \
  0x02 /*!< Commits changes to the \ref PowerField output property */
#define UF_UPDATE_TACHO_LIMIT                                                  \
  0x04 /*!< Commits changes to the \ref TachoLimitField output property */
...
#define UF_UPDATE_PID_VALUES                                                   \
  0x10 /*!< Commits changes to the PID motor regulation properties */
#define UF_UPDATE_RESET_BLOCK_COUNT                                            \
  0x20 /*!< Resets the NXT-G block-relative rotation counter */
#define UF_UPDATE_RESET_ROTATION_COUNT                                         \
  0x40 /*!< Resets the program-relative (user) rotation counter */
...
#define RESET_BLOCKANDTACHO                                                    \
  0x28 /*!< Reset both the internal counter and the NXT-G block counter */
...
#define OUT_MODE_COAST                                                         \
  0x00 /*!< No power and no braking so motors rotate freely. */
#define OUT_MODE_MOTORON                                                       \
  0x01 /*!< Enables PWM power to the outputs given the power setting */
#define OUT_MODE_BRAKE 0x02 /*!< Uses electronic braking to outputs */
#define OUT_MODE_REGULATED                                                     \
  0x04 /*!< Enables active power regulation using the regulation mode value */
 ...
 * @{
 */
#define OUT_REGOPTION_NO_SATURATION                                            \
  0x01    /*!< Do not limit intermediary regulation results */
 ...
 * or position regulation (requires the enhanced NBC/NXC firmware
 * version 1.31+).
 * \sa SetOutput()
 * @{
 */
 ...
 * @{
 */
/** Update flags field. Contains a combination of the update flag constants.
 * Read/write. Use \ref UF_UPDATE_MODE, \ref UF_UPDATE_SPEED, \ref
 * UF_UPDATE_TACHO_LIMIT, and \ref UF_UPDATE_PID_VALUES along with other fields
 * to commit changes to the state of outputs. Set the appropriate flags after
 * setting one or more of the output fields in order for the changes to actually
 *  go into affect. */
#define UpdateFlagsField 0
/** Mode field. Contains a combination of the output mode constants. Read/write.
 *  The \ref OUT_MODE_MOTORON bit must be set in order for power to be applied
 * to the motors. Add \ref OUT_MODE_BRAKE to enable electronic braking. Braking
 * means that the output voltage is not allowed to float between active PWM
 * pulses. It improves the accuracy of motor output but uses more battery power.
 *  To use motor regulation include \ref OUT_MODE_REGULATED in the \ref
 * OutputModeField value. Use
 *  \ref UF_UPDATE_MODE with \ref UpdateFlagsField to commit changes to this
 * field. */
#define OutputModeField 1
/** Power field. Contains the desired power level (-100 to 100). Read/write.
 *  Specify the power level of the output. The absolute value of PowerField is a
 * percentage of the full power of the motor. The sign of PowerField controls
 * the rotation direction. Positive values tell the firmware to turn the motor
 * forward, while negative values turn the motor backward. Use \ref
 * UF_UPDATE_SPEED with \ref UpdateFlagsField to commit changes to this field.
 */
#define PowerField 2
/** Actual speed field. Contains the actual power level (-100 to 100). Read
 * only. Return the percent of full power the firmware is applying to the
 * output. This may vary from the PowerField value when auto-regulation code in
 * the firmware responds to a load on the output. */
#define ActualSpeedField 3
/** Internal tachometer count field. Contains the current internal tachometer
 * count. Read only. Return the internal position counter value for the
 * specified output. The internal count is reset automatically when a new goal
 * is set using the \ref TachoLimitField and the \ref UF_UPDATE_TACHO_LIMIT
 * flag. Set the \ref UF_UPDATE_RESET_COUNT flag in \ref UpdateFlagsField to
 * reset TachoCountField and cancel any \ref TachoLimitField. The sign of
 * TachoCountField indicates the motor rotation direction. */
 ...
 *  Use \ref UF_UPDATE_TACHO_LIMIT with the \ref UpdateFlagsField field to
 * commit changes to the TachoLimitField. The value of this field is a relative
 * distance from the current motor position at the moment when the \ref
 ...
 *  Use this field to specify the running state of an output. Set the
 * RunStateField to \ref OUT_RUNSTATE_RUNNING to enable power to any output. Use
 * \ref OUT_RUNSTATE_RAMPUP to enable automatic ramping to a new \ref PowerField
 *  level greater than the current \ref PowerField level. Use \ref
 * OUT_RUNSTATE_RAMPDOWN to enable automatic ramping to a new \ref PowerField
 * level less than the current \ref PowerField level. Both the rampup and
 * rampdown bits must be used in conjunction with appropriate \ref
 * TachoLimitField and \ref PowerField values. In this case the firmware
 * smoothly increases or decreases the actual power to the new \ref PowerField
 *  level over the total number of degrees of rotation specified in \ref
 * TachoLimitField. */
#define RunStateField 6
/** Turn ratio field. Contains the current turn ratio. Only applicable when
 * synchronizing multiple motors. Read/write. Use this field to specify a
 * proportional turning ratio. This field must be used in conjunction with other
 *  field values: \ref OutputModeField must include \ref OUT_MODE_MOTORON and
 * \ref OUT_MODE_REGULATED, \ref RegModeField must be set to
 *  \ref OUT_REGMODE_SYNC, \ref RunStateField must not be \ref
 * OUT_RUNSTATE_IDLE, and \ref PowerField must be non-zero. There are only three
 * valid combinations of left and right motors for use with TurnRatioField: \ref
 * OUT_AB, \ref OUT_BC, and \ref OUT_AC. In each of these three options the
 * first motor listed is considered to be the left motor and the second motor is
 * the right motor, regardless of the physical configuration of the robot.
 *  Negative turn ratio values shift power toward the left motor while positive
 * values shift power toward the right motor. An absolute value of 50 usually
 * results in one motor stopping. An absolute value of 100 usually results in
 * two motors turning in opposite directions at equal power. */
#define TurnRatioField 7
/** Regulation mode field. Contains one of the regulation mode constants.
 * Read/write. This field specifies the regulation mode to use with the
 * specified port(s). It is ignored if the \ref OUT_MODE_REGULATED bit is not
 * set in the \ref OutputModeField field. Unlike \ref OutputModeField,
 * RegModeField is not a bitfield. Only one regulation mode value can be set at
 * a time. Speed regulation means that the firmware tries to maintain a certain
 * speed based on the \ref PowerField setting. The firmware adjusts the PWM duty
 * cycle if the motor is affected by a physical load. This adjustment is
 *  reflected by the value of the \ref ActualSpeedField property. When using
 * speed regulation, do not set \ref PowerField to its maximum value since the
 * firmware cannot adjust to higher power levels in that situation.
 *  Synchronization means the firmware tries to keep two motors in sync
 * regardless of physical loads. Use this mode to maintain a straight path for a
 * mobile robot automatically. Also use this mode with the
 *  \ref TurnRatioField property to provide proportional turning.
 *  Set \ref OUT_REGMODE_SYNC on at least two motor ports in order for
 * synchronization to function. Setting
 *  \ref OUT_REGMODE_SYNC on all three motor ports will result in only the first
 * two (\ref OUT_A and \ref OUT_B) being synchronized. */
#define RegModeField 8
/** Overload field. Contains a boolean value which is TRUE if the motor is
 * overloaded. Read only. This field will have a value of 1 (true) if the
 * firmware speed regulation cannot overcome a physical load on the motor. In
 * other words, the motor is turning more slowly than expected. If the motor
 * speed can be maintained in spite of loading then this field value is zero
 * (false). In order to use this field the motor must have a non-idle \ref
 * RunStateField, an \ref OutputModeField which includes
 *  \ref OUT_MODE_MOTORON and \ref OUT_MODE_REGULATED, and its \ref RegModeField
 * must be set to \ref OUT_REGMODE_SPEED. */
#define OverloadField 9
/** Proportional field. Contains the proportional constant for the PID motor
 * controller. Read/write. This field specifies the proportional term used in
 * the internal proportional-integral-derivative (PID) control algorithm. Set
 * \ref UF_UPDATE_PID_VALUES to commit changes to RegPValue, RegIValue, and
 * RegDValue simultaneously. */
#define RegPValueField 10
/** Integral field. Contains the integral constant for the PID motor controller.
 * Read/write. This field specifies the integral term used in the internal
 * proportional-integral-derivative (PID) control algorithm. Set \ref
 * UF_UPDATE_PID_VALUES to commit changes to RegPValue, RegIValue, and RegDValue
 * simultaneously. */
#define RegIValueField 11
/** Derivative field. Contains the derivative constant for the PID motor
 * controller. Read/write. This field specifies the derivative term used in the
 * internal proportional-integral-derivative (PID) control algorithm. Set \ref
 * UF_UPDATE_PID_VALUES to commit changes to RegPValue, RegIValue, and RegDValue
 * simultaneously. */
#define RegDValueField 12
/** NXT-G block tachometer count field. Contains the current NXT-G block
 * tachometer count. Read only. Return the block-relative position counter value
 * for the specified port. Refer to the \ref UpdateFlagsField description for
 * information about how to use block-relative position counts. Set the \ref
 * UF_UPDATE_RESET_BLOCK_COUNT flag in \ref UpdateFlagsField to request that the
 * firmware reset the BlockTachoCountField. The sign of BlockTachoCountField
 * indicates the direction of rotation. Positive values indicate forward
 * rotation and negative values indicate reverse rotation. Forward and reverse
 * depend on the orientation of the motor. */
 ...
 *  Refer to the \ref UpdateFlagsField description for information about how to
 * use program-relative position counts. Set the \ref
 * UF_UPDATE_RESET_ROTATION_COUNT flag in \ref UpdateFlagsField to request that
 * the firmware reset the RotationCountField. The sign of RotationCountField
 * indicates the direction of rotation. Positive values indicate forward
 *  rotation and negative values indicate reverse rotation. Forward and reverse
 * depend on the orientation of the motor. */
#define RotationCountField 14
#if defined(__ENHANCED_FIRMWARE) && (__FIRMWARE_VERSION > 107)
/** Options field. Contains a combination of the output options constants.
 * Read/write. Set options for how the output module will act when a tachometer
 * limit is reached. Option constants can be combined with bitwise OR.  Use
 * OUT_OPTION_HOLDATLIMIT to have the output module hold the motor when it
 * reaches the tachometer limit.  Use OUT_OPTION_RAMPDOWNTOLIMIT to have the
 * output module ramp down the motor power as it approaches the tachometer
 * limit.
 */
...
/** MaxAcceleration field. Contains the current max acceleration value.
 * Read/write. Set the maximum acceleration to be used during position
 * regulation.
 */
...
#define OutputOffsetMotorRPM(p)                                                \
  (((p) * 32) +                                                                \
   16) /*!< Not updated, will be removed later !! (2 bytes) sword */
...
#define OutputOffsetMode(p)                                                    \
  (((p) * 32) + 19) /*!< RW - Holds motor mode: Run, Break, regulated, ... (1  \
                       byte) ubyte */
#define OutputOffsetSpeed(p)                                                   \
  (((p) * 32) + 20) /*!< RW - Holds the wanted speed (1 byte) sbyte */
#define OutputOffsetActualSpeed(p)                                             \
  (((p) * 32) + 21) /*!< R  - Holds the current motor speed (1 byte) sbyte */
#define OutputOffsetRegPParameter(p)                                           \
  (((p) * 32) +                                                                \
   22) /*!< RW - Holds the P-constant used in the regulation (1 byte) ubyte */
#define OutputOffsetRegIParameter(p)                                           \
  (((p) * 32) +                                                                \
   23) /*!< RW - Holds the I-constant used in the regulation (1 byte) ubyte */
#define OutputOffsetRegDParameter(p)                                           \
  (((p) * 32) +                                                                \
   24) /*!< RW - Holds the D-constant used in the regulation (1 byte) ubyte */
#define OutputOffsetRunState(p)                                                \
  (((p) * 32) + 25) /*!< RW - Holds the current motor run state in the output  \
                       module (1 byte) ubyte */
#define OutputOffsetRegMode(p)                                                 \
  (((p) * 32) +                                                                \
   26) /*!< RW - Tells which regulation mode should be used (1 byte) ubyte */
...
#define OutputOffsetRegulationTime                                             \
  96 /*!< use for frequency of checking regulation mode (1 byte) ubyte         \
        (NBC/NXC) */
#if defined(__ENHANCED_FIRMWARE) && (__FIRMWARE_VERSION > 107)
#define OutputOffsetRegulationOptions                                          \
  97 /*!< use for position regulation options (1 byte) ubyte (NBC/NXC) */
#endif
 ...
 * @{
 */
#define COM_CHANNEL_NONE_ACTIVE                                                \
  0x00 /*!< None of the low speed channels are active */
...
#define LOWSPEED_TX_ERROR                                                      \
  2 /*!< Lowspeed port encountered an error while transmitting data */
#define LOWSPEED_RX_ERROR                                                      \
  3       /*!< Lowspeed port encountered an error while receiving data */
...
#define LowSpeedOffsetInBufBuf(p)                                              \
  (((p) * 19) + 0) /*!< RW - Input buffer data buffer field offset (16 bytes)  \
                    */
#define LowSpeedOffsetInBufInPtr(p)                                            \
  (((p) * 19) + 16) /*!< RW - Input buffer in pointer field offset (1 byte) */
...
#define LowSpeedOffsetInBufBytesToRx(p)                                        \
  (((p) * 19) +                                                                \
   18) /*!< RW - Input buffer bytes to receive field offset (1 byte) */

#define LowSpeedOffsetOutBufBuf(p)                                             \
  (((p) * 19) +                                                                \
   76) /*!< RW - Output buffer data buffer field offset (16 bytes) */
...
#define LowSpeedOffsetOutBufBytesToRx(p)                                       \
  (((p) * 19) +                                                                \
   94) /*!< RW - Output buffer bytes to receive field offset (1 byte) */

#define LowSpeedOffsetMode(p)                                                  \
  ((p) + 152) /*!< R - Lowspeed port mode (1 byte) */
#define LowSpeedOffsetChannelState(p)                                          \
  ((p) + 156) /*!< R - Lowspeed channgel state (1 byte) */
#define LowSpeedOffsetErrorType(p)                                             \
  ((p) + 160) /*!< R - Lowspeed port error type (1 byte) */
 ...
 */
#ifdef __ENHANCED_FIRMWARE
#define LSREAD_RESTART_ALL                                                     \
  0x00 /*!< Restart on read for all channels (default) */
...
#define US_CMD_SINGLESHOT                                                      \
  0x01 /*!< Command to put the ultrasonic sensor into single shot mode */
#define US_CMD_CONTINUOUS                                                      \
  0x02 /*!< Command to put the ultrasonic sensor into continuous polling mode  \
          (default) */
#define US_CMD_EVENTCAPTURE                                                    \
  0x03 /*!< Command to put the ultrasonic sensor into event capture mode */
#define US_CMD_WARMRESET                                                       \
  0x04 /*!< Command to warm reset the ultrasonic sensor */

#define US_REG_CM_INTERVAL                                                     \
  0x40 /*!< The register address used to store the CM interval */
#define US_REG_ACTUAL_ZERO                                                     \
  0x50 /*!< The register address used to store the actual zero value */
#define US_REG_SCALE_FACTOR                                                    \
  0x51 /*!< The register address used to store the scale factor value */
#define US_REG_SCALE_DIVISOR                                                   \
  0x52 /*!< The register address used to store the scale divisor value */
 ...
 */
// R1/R0
#define TEMP_RES_9BIT                                                          \
  0x00 /*!< Set the temperature conversion resolution to 9 bit */
#define TEMP_RES_10BIT                                                         \
  0x20 /*!< Set the temperature conversion resolution to 10 bit */
#define TEMP_RES_11BIT                                                         \
  0x40 /*!< Set the temperature conversion resolution to 11 bit */
#define TEMP_RES_12BIT                                                         \
  0x60 /*!< Set the temperature conversion resolution to 12 bit */
// SD (shutdown mode)
...
#define TEMP_REG_TEMP                                                          \
  0x00 /*!< The register where temperature values can be read */
#define TEMP_REG_CONFIG                                                        \
  0x01 /*!< The register for reading/writing sensor configuration values */
 ...
 * @{
 */
#define DISPLAY_ERASE_ALL                                                      \
  0x00 /*!< W - erase entire screen     (CMD,x,x,x,x,x) */
#define DISPLAY_PIXEL                                                          \
  0x01 /*!< W - set pixel (on/off)      (CMD,TRUE/FALSE,X,Y,x,x) */
#define DISPLAY_HORIZONTAL_LINE                                                \
  0x02 /*!< W - draw horizontal line    (CMD,TRUE/FALSE,X1,Y1,X2,x) */
#define DISPLAY_VERTICAL_LINE                                                  \
  0x03 /*!< W - draw vertical line      (CMD,TRUE/FALSE,X1,Y1,x,Y2) */
#define DISPLAY_CHAR                                                           \
  0x04 /*!< W - draw char (actual font) (CMD,TRUE,X1,Y1,Char,x) */
#define DISPLAY_ERASE_LINE                                                     \
  0x05 /*!< W - erase a single line     (CMD,x,LINE,x,x,x) */
#define DISPLAY_FILL_REGION                                                    \
  0x06 /*!< W - fill screen region      (CMD,TRUE/FALSE,X1,Y1,X2,Y2) */
#define DISPLAY_FRAME                                                          \
  0x07    /*!< W - draw a frame (on/off)   (CMD,TRUE/FALSE,X1,Y1,X2,Y2) */
/** @} */ // end of DisplayExecuteFunctionConstants group

/** @defgroup DisplayDrawOptionConstants Drawing option constants
 * Constants that are for specifying drawing options in several display module
 * API functions. Bits 0 & 1 (values 0,1,2,3) control screen clearing behaviour
 * (Not within RIC files). Bit 2 (value 4) controls the NOT operation, i.e. draw
 * in white or invert text/graphics. Bits 3 & 4 (values 0,8,16,24) control pixel
 * logical combinations (COPY/AND/OR/XOR). Bit 5 (value 32) controls shape
 * filling, or overrides text/graphic bitmaps with set pixels. These may be ORed
 * together for the full instruction (e.g.,
 * DRAW_OPT_NORMAL|DRAW_OPT_LOGICAL_XOR) These operations are resolved into the
 * separate, common parameters defined in 'c_display.iom' before any drawing
 * function is called. Note that when drawing a RIC file, the initial
 * 'DrawingOptions' parameter supplied in the drawing instruction controls
 * screen clearing, but nothing else. The 'CopyOptions' parameter from each
 * instruction in the RIC file then controls graphic operations, but the
 * screen-clearing bits are ignored.
...
#define DRAW_OPT_CLEAR_WHOLE_SCREEN                                            \
  (0x0001) /*!< Clear the entire screen before drawing */
...
#define DRAW_OPT_CLEAR_PIXELS                                                  \
  (0x0004) /*!< Clear pixels while drawing (aka draw in white) */
#define DRAW_OPT_CLEAR                                                         \
  (0x0004) /*!< Clear pixels while drawing (aka draw in white) */
#define DRAW_OPT_INVERT (0x0004) /*!< Invert text or graphics */

#define DRAW_OPT_LOGICAL_COPY                                                  \
  (0x0000) /*!< Draw pixels using a logical copy operation */
#define DRAW_OPT_LOGICAL_AND                                                   \
  (0x0008) /*!< Draw pixels using a logical AND operation */
#define DRAW_OPT_LOGICAL_OR                                                    \
  (0x0010) /*!< Draw pixels using a logical OR operation */
#define DRAW_OPT_LOGICAL_XOR                                                   \
  (0x0018) /*!< Draw pixels using a logical XOR operation */

#define DRAW_OPT_FILL_SHAPE                                                    \
  (0x0020) /*!< Fill the shape while drawing (rectangle, circle, ellipses, and \
              polygon) */

#define DRAW_OPT_CLEAR_SCREEN_MODES                                            \
  (0x0003) /*!< Bit mask for the clear screen modes */
#define DRAW_OPT_LOGICAL_OPERATIONS                                            \
  (0x0018) /*!< Bit mask for the logical drawing operations */
 ...
 * @{
 */
#define DRAW_OPT_FONT_DIRECTIONS                                               \
  (0x01C0) /*!< Bit mask for the font direction bits */
 ...
 * \sa SetDisplayNormal(), GetDisplayNormal(), SetDisplayPopup(),
 * GetDisplayPopup()
 * @{
 */
...
#define DisplayOffsetPTextLines(p)                                             \
  (((p) * 4) + 16)                  /*!< Pointer to text strings */
#define DisplayOffsetPStatusText 48 /*!< Pointer to status text string */
#define DisplayOffsetPStatusIcons                                              \
  52 /*!< Pointer to status icon collection file */
#define DisplayOffsetPScreens(p)                                               \
  (((p) * 4) + 56) /*!< Pointer to screen bitmap file */
#define DisplayOffsetPBitmaps(p)                                               \
  (((p) * 4) + 68) /*!< Pointer to free bitmap files */
#define DisplayOffsetPMenuText                                                 \
  84 /*!< Pointer to menu icon text (NULL == none) */
#define DisplayOffsetPMenuIcons(p)                                             \
  (((p) * 4) + 88) /*!< Pointer to menu icon images (NULL == none) */
#define DisplayOffsetPStepIcons                                                \
  100 /*!< Pointer to step icon collection file                                \
       */
#define DisplayOffsetDisplay                                                   \
  104 /*!< Display content copied to physical display every 17 mS */
#define DisplayOffsetStatusIcons(p)                                            \
  ((p) + 108) /*!< Index in status icon collection file (index = 0 -> none) */
#define DisplayOffsetStepIcons(p)                                              \
  ((p) + 112) /*!< Index in step icon collection file (index = 0 -> none) */
...
#define DisplayOffsetNormal(l, w)                                              \
  (((l) * 100) + (w) + 119) /*!< Raw display memory for normal screen */
#define DisplayOffsetPopup(l, w)                                               \
  (((l) * 100) + (w) + 919) /*!< Raw display memory for popup screen */

#if defined(__ENHANCED_FIRMWARE) && (__FIRMWARE_VERSION > 107)
#define DisplayOffsetContrast                                                  \
  1719 /*!< Adjust the display contrast with this field */
#endif
...
#define USB_PROTOCOL_OVERHEAD                                                  \
  2 /*!< Size of USB Overhead in bytes -- Command type byte + Command*/
...
#define BT_DEFAULT_INQUIRY_MAX                                                 \
  0 /*!< Bluetooth default inquiry Max (0 == unlimited)*/
#define BT_DEFAULT_INQUIRY_TIMEOUT_LO                                          \
  15      /*!< Bluetooth inquiry timeout (15*1.28 sec = 19.2 sec) */
...
#define DATA_MODE_UPDATE                                                       \
  0x08    /*!< Indicates that the data mode has been changed. */
...
#define BT_CONNECTION_0_ENABLE                                                 \
  0x10 /*!< BtStateStatus connection 0 enable/disable bit */
#define BT_CONNECTION_1_ENABLE                                                 \
  0x20 /*!< BtStateStatus connection 1 enable/disable bit */
#define BT_CONNECTION_2_ENABLE                                                 \
  0x40 /*!< BtStateStatus connection 2 enable/disable bit */
#define BT_CONNECTION_3_ENABLE                                                 \
  0x80    /*!< BtStateStatus connection 3 enable/disable bit */
...
#define CONN_HS_ALL                                                            \
  0x4 /*!< RS485 (hi-speed) connection (port 4, all devices) */
#define CONN_HS_1                                                              \
  0x5 /*!< RS485 (hi-speed) connection (port 4, device address 1) */
#define CONN_HS_2                                                              \
  0x6 /*!< RS485 (hi-speed) connection (port 4, device address 2) */
#define CONN_HS_3                                                              \
  0x7 /*!< RS485 (hi-speed) connection (port 4, device address 3) */
#define CONN_HS_4                                                              \
  0x8 /*!< RS485 (hi-speed) connection (port 4, device address 4) */
#define CONN_HS_5                                                              \
  0x9 /*!< RS485 (hi-speed) connection (port 4, device address 5) */
#define CONN_HS_6                                                              \
  0xa /*!< RS485 (hi-speed) connection (port 4, device address 6) */
#define CONN_HS_7                                                              \
  0xb /*!< RS485 (hi-speed) connection (port 4, device address 7) */
#define CONN_HS_8                                                              \
  0xc     /*!< RS485 (hi-speed) connection (port 4, device address 8) */
 ...
 * @{
 */
#define HS_MODE_DEFAULT                                                        \
  HS_MODE_8N1 /*!< HsMode default mode (8 data bits, no parity, 1 stop bit) */
 ...
 */
#define HS_MODE_8N1                                                            \
  (HS_MODE_8_DATA | HS_MODE_N_PARITY |                                         \
   HS_MODE_10_STOP) /*!< HsMode 8 data bits, no parity, 1 stop bit */
#define HS_MODE_7E1                                                            \
  (HS_MODE_7_DATA | HS_MODE_E_PARITY |                                         \
   HS_MODE_10_STOP) /*!< HsMode 7 data bits, even parity, 1 stop bit */
 ...
 * Constants for all the Comm module interface functions executable via
...
#define BT_CMD_READY                                                           \
  0x02 /*!< A constant representing bluetooth direct command */
#define HS_CMD_READY                                                           \
  0x04    /*!< A constant representing high speed direct command */
 ...
 * @{
 */
#define CommOffsetPFunc                                                        \
  0 /*!< Offset to the Comm module first function pointer (4 bytes) */
#define CommOffsetPFuncTwo                                                     \
  4 /*!< Offset to the Comm module second function pointer (4 bytes) */
// BtDeviceTable[30] (930 bytes)
#define CommOffsetBtDeviceTableName(p)                                         \
  (((p) * 31) + 8) /*!< Offset to BT device table name (16 bytes) */
#define CommOffsetBtDeviceTableClassOfDevice(p)                                \
  (((p) * 31) +                                                                \
   24) /*!< Offset to Bluetooth device table device class (4 bytes) */
#define CommOffsetBtDeviceTableBdAddr(p)                                       \
  (((p) * 31) + 28) /*!< Offset to Bluetooth device table address (7 bytes) */
...
#define CommOffsetBtConnectTableName(p)                                        \
  (((p) * 47) + 938) /*!< Offset to Bluetooth connect table name (16 bytes) */
#define CommOffsetBtConnectTableClassOfDevice(p)                               \
  (((p) * 47) +                                                                \
   954) /*!< Offset to Bluetooth connect table device class (4 bytes) */
#define CommOffsetBtConnectTablePinCode(p)                                     \
  (((p) * 47) +                                                                \
   958) /*!< Offset to Bluetooth connect table pin code (16 bytes) */
...
#define CommOffsetBtConnectTableStreamStatus(p)                                \
  (((p) * 47) +                                                                \
   982) /*!< Offset to Bluetooth connect table stream status (1 byte) */
#define CommOffsetBtConnectTableLinkQuality(p)                                 \
  (((p) * 47) +                                                                \
   983) /*!< Offset to Bluetooth connect table link quality (1 byte) */
// General brick data
//   BRICKDATA      BrickData; (31 bytes)
#define CommOffsetBrickDataName 1126 /*!< Offset to brick name (16 bytes) */
#define CommOffsetBrickDataBluecoreVersion                                     \
  1142 /*!< Offset to Bluecore version (2 bytes) */
#define CommOffsetBrickDataBdAddr                                              \
  1144 /*!< Offset to Bluetooth address (7 bytes) */
#define CommOffsetBrickDataBtStateStatus                                       \
  1151 /*!< Offset to BtStateStatus (1 byte) */
#define CommOffsetBrickDataBtHwStatus                                          \
  1152 /*!< Offset to BtHwStatus (1 byte)                                      \
        */
#define CommOffsetBrickDataTimeOutValue                                        \
  1153 /*!< Offset to data timeout value (1 byte) */
//  BTBUF          BtInBuf; (132 bytes)
#define CommOffsetBtInBufBuf                                                   \
  1157 /*!< Offset to Bluetooth input buffer data (128 bytes) */
#define CommOffsetBtInBufInPtr                                                 \
  1285 /*!< Offset to Bluetooth input buffer front pointer (1 byte) */
#define CommOffsetBtInBufOutPtr                                                \
  1286 /*!< Offset to Bluetooth output buffer back pointer (1 byte) */
//  BTBUF          BtOutBuf; (132 bytes)
#define CommOffsetBtOutBufBuf                                                  \
  1289 /*!< Offset to Bluetooth output buffer offset data (128 bytes) */
#define CommOffsetBtOutBufInPtr                                                \
  1417 /*!< Offset to Bluetooth output buffer front pointer (1 byte) */
#define CommOffsetBtOutBufOutPtr                                               \
  1418 /*!< Offset to Bluetooth output buffer back pointer (1 byte) */
// HI Speed related entries
//  HSBUF          HsInBuf; (132 bytes)
#define CommOffsetHsInBufBuf                                                   \
  1421 /*!< Offset to High Speed input buffer data (128 bytes) */
#define CommOffsetHsInBufInPtr                                                 \
  1549 /*!< Offset to High Speed input buffer front pointer (1 byte) */
#define CommOffsetHsInBufOutPtr                                                \
  1550 /*!< Offset to High Speed input buffer back pointer (1 byte) */
//  HSBUF          HsOutBuf; (132 bytes)
#define CommOffsetHsOutBufBuf                                                  \
  1553 /*!< Offset to High Speed output buffer data (128 bytes) */
#define CommOffsetHsOutBufInPtr                                                \
  1681 /*!< Offset to High Speed output buffer front pointer (1 byte) */
#define CommOffsetHsOutBufOutPtr                                               \
  1682 /*!< Offset to High Speed output buffer back pointer (1 byte) */
// USB related entries
//  USBBUF         UsbInBuf; (68 bytes)
#define CommOffsetUsbInBufBuf                                                  \
  1685 /*!< Offset to Usb input buffer data (64 bytes) */
#define CommOffsetUsbInBufInPtr                                                \
  1749 /*!< Offset to Usb input buffer front pointer (1 byte) */
#define CommOffsetUsbInBufOutPtr                                               \
  1750 /*!< Offset to Usb input buffer back pointer (1 byte) */
//  USBBUF         UsbOutBuf; (68 bytes)
#define CommOffsetUsbOutBufBuf                                                 \
  1753 /*!< Offset to Usb output buffer data (64 bytes) */
#define CommOffsetUsbOutBufInPtr                                               \
  1817 /*!< Offset to Usb output buffer front pointer (1 byte) */
#define CommOffsetUsbOutBufOutPtr                                              \
  1818 /*!< Offset to Usb output buffer back pointer (1 byte) */
//  USBBUF         UsbPollBuf; (68 bytes)
#define CommOffsetUsbPollBufBuf                                                \
  1821 /*!< Offset to Usb Poll buffer data (64 bytes) */
#define CommOffsetUsbPollBufInPtr                                              \
  1885 /*!< Offset to Usb Poll buffer front pointer (1 byte) */
#define CommOffsetUsbPollBufOutPtr                                             \
  1886 /*!< Offset to Usb Poll buffer back pointer (1 byte) */

#define CommOffsetBtDeviceCnt                                                  \
  1889 /*!< Offset to Bluetooth device count (1 byte) */
#define CommOffsetBtDeviceNameCnt                                              \
  1890 /*!< Offset to Bluetooth device name count (1 byte) */
...
#define CommOffsetBtDataMode                                                   \
  1898 /*!< Offset to Bluetooth data mode (1 byte)                             \
        */
#define CommOffsetHsDataMode                                                   \
  1899 /*!< Offset to High Speed data mode (1 byte) */
#endif
...
#define RCX_DatalogSrcIndirectSrc                                              \
  37 /*!< The RCX indirect datalog source source */
#define RCX_DatalogSrcDirectSrc                                                \
  38 /*!< The RCX direct datalog source source                                 \
      */
#define RCX_DatalogValueIndirectSrc                                            \
  39 /*!< The RCX indirect datalog value source */
...
/** @defgroup HTIRLinkPFConstants HiTechnic/mindsensors Power Function/IR Train
 * constants Constants that are for use with the HiTechnic IRLink or mindsensors
 * nRLink in Power Function or IR Train mode.
 * @{
 */
...
#define PF_MODE_SINGLE_PIN_CONT                                                \
  2 /*!< Power function mode single pin continuous */
#define PF_MODE_SINGLE_PIN_TIME 3 /*!< Power function mode single pin timed */
#define PF_MODE_COMBO_PWM                                                      \
  4 /*!< Power function mode combo pulse width modulation (PWM) */
#define PF_MODE_SINGLE_OUTPUT_PWM                                              \
  4 /*!< Power function mode single output pulse width modulation (PWM) */
#define PF_MODE_SINGLE_OUTPUT_CST                                              \
  6       /*!< Power function mode single output clear, set, toggle (CST) */
...
#define HT_CMD_COLOR2_BLCAL                                                    \
  0x42 /*!< Set the Color2 sensor to black level calibration mode */
#define HT_CMD_COLOR2_WBCAL                                                    \
  0x43 /*!< Set the Color2 sensor to white level calibration mode */
...
#define HTANGLE_MODE_CALIBRATE                                                 \
  0x43 /*!< Resets 0 degree position to current shaft angle */
#define HTANGLE_MODE_RESET 0x52 /*!< Resets the accumulated angle */

#define HTANGLE_REG_MODE 0x41 /*!< Angle mode register */
#define HTANGLE_REG_DCDIR                                                      \
  0x42 /*!< Angle current angle (2 degree increments) register */
#define HTANGLE_REG_DC01                                                       \
  0x43 /*!< Angle current angle (1 degree adder) register */
#define HTANGLE_REG_DC02                                                       \
  0x44 /*!< Angle 32 bit accumulated angle, high byte register */
#define HTANGLE_REG_DC03                                                       \
  0x45 /*!< Angle 32 bit accumulated angle, mid byte register */
#define HTANGLE_REG_DC04                                                       \
  0x46 /*!< Angle 32 bit accumulated angle, mid byte register */
#define HTANGLE_REG_DC05                                                       \
  0x47 /*!< Angle 32 bit accumulated angle, low byte register */
#define HTANGLE_REG_DCAVG                                                      \
  0x48 /*!< Angle 16 bit revolutions per minute, high byte register */
#define HTANGLE_REG_ACDIR                                                      \
  0x49    /*!< Angle 16 bit revolutions per minute, low byte register */
...
#define MS_ADDR_NXTSERVO_EM                                                    \
  0x40 /*!< MindSensors NXTServo in edit macro mode I2C address */
...
#define MS_ADDR_IVSENS                                                         \
  0x12 /*!< MindSensors IVSens (NXTPowerMeter) I2C address */
...
#define DIST_REG_NUM_POINTS                                                    \
  0x51 /*!< The DIST-Nx number of data points in Custom curve register */
...
#define ACCL_CMD_X_CAL_END                                                     \
  0x78 /*!< Acquire X-axis calibration point and end calibration */
#define ACCL_CMD_Y_CAL_END                                                     \
  0x79 /*!< Acquire Y-axis calibration point and end calibration */
#define ACCL_CMD_Z_CAL_END                                                     \
  0x7a /*!< Acquire Z-axis calibration point and end calibration */
...
#define NXTSERVO_REG_S1_QPOS                                                   \
  0x5A /*!< NXTServo servo 1 quick position register. (write only) */
#define NXTSERVO_REG_S2_QPOS                                                   \
  0x5B /*!< NXTServo servo 2 quick position register. (write only) */
#define NXTSERVO_REG_S3_QPOS                                                   \
  0x5C /*!< NXTServo servo 3 quick position register. (write only) */
#define NXTSERVO_REG_S4_QPOS                                                   \
  0x5D /*!< NXTServo servo 4 quick position register. (write only) */
#define NXTSERVO_REG_S5_QPOS                                                   \
  0x5E /*!< NXTServo servo 5 quick position register. (write only) */
#define NXTSERVO_REG_S6_QPOS                                                   \
  0x5F /*!< NXTServo servo 6 quick position register. (write only) */
#define NXTSERVO_REG_S7_QPOS                                                   \
  0x60 /*!< NXTServo servo 7 quick position register. (write only) */
#define NXTSERVO_REG_S8_QPOS                                                   \
  0x61 /*!< NXTServo servo 8 quick position register. (write only) */

#define NXTSERVO_EM_REG_CMD                                                    \
  0x00 /*!< NXTServo in macro edit mode command register. */
#define NXTSERVO_EM_REG_EEPROM_START                                           \
  0x21 /*!< NXTServo in macro edit mode EEPROM start register. */
#define NXTSERVO_EM_REG_EEPROM_END                                             \
  0xFF    /*!< NXTServo in macro edit mode EEPROM end register. */
 ...
 * @{
 */
#define NXTSERVO_QPOS_CENTER                                                   \
  150                        /*!< Center quick position for 1500us servos. */
...
#define NXTSERVO_CMD_HALT                                                      \
  0x48 /*!< Halt Macro. This command re-initializes the macro environment. */
...
#define NXTSERVO_CMD_EDIT1                                                     \
  0x45 /*!< Edit Macro (part 1 of 2 character command sequence) */
#define NXTSERVO_CMD_EDIT2                                                     \
  0x4D /*!< Edit Macro (part 2 of 2 character command sequence) */
 ...
 * @{
 */
#define NXTHID_REG_CMD                                                         \
  0x41 /*!< NXTHID command register.  See \ref NXTHIDCommands group. */
#define NXTHID_REG_MODIFIER                                                    \
  0x42 /*!< NXTHID modifier register.  See \ref NXTHIDModifiers group. */
...
#define NXTHID_CMD_ASCII                                                       \
  0x41 /*!< Use ASCII data mode. In ASCII mode no non-printable characters can \
          be sent. */
#define NXTHID_CMD_DIRECT                                                      \
  0x44 /*!< Use direct data mode In direct mode any character can be sent. */
...
#define NXTPM_REG_CURRENT                                                      \
  0x42 /*!< NXTPowerMeter present current in mA register. (2 bytes) */
#define NXTPM_REG_VOLTAGE                                                      \
  0x44 /*!< NXTPowerMeter present voltage in mV register. (2 bytes) */
#define NXTPM_REG_CAPACITY                                                     \
  0x46 /*!< NXTPowerMeter capacity used since last reset register. (2 bytes)   \
        */
#define NXTPM_REG_POWER                                                        \
  0x48 /*!< NXTPowerMeter present power register. (2 bytes) */
#define NXTPM_REG_TOTALPOWER                                                   \
  0x4A /*!< NXTPowerMeter total power consumed since last reset register. (4   \
          bytes) */
#define NXTPM_REG_MAXCURRENT                                                   \
  0x4E /*!< NXTPowerMeter max current register. (2 bytes) */
#define NXTPM_REG_MINCURRENT                                                   \
  0x50 /*!< NXTPowerMeter min current register. (2 bytes) */
#define NXTPM_REG_MAXVOLTAGE                                                   \
  0x52 /*!< NXTPowerMeter max voltage register. (2 bytes) */
#define NXTPM_REG_MINVOLTAGE                                                   \
...
#define NXTPM_REG_ERRORCOUNT                                                   \
  0x5F    /*!< NXTPowerMeter error count register. (2 bytes) */
/** @} */ // end of NXTPowerMeterRegisters group

/** @defgroup NXTPowerMeterCommands MindSensors NXTPowerMeter commands
 * NXTPowerMeter device command constants. These are written to the command
 * register to control the device.
 * @{
 */
...
#define NXTLL_REG_RESULT                                                       \
  0x44 /*!< NXTLineLeader result register (sensor bit values). */
#define NXTLL_REG_SETPOINT                                                     \
  0x45 /*!< NXTLineLeader user settable average (setpoint) register. Default   \
          = 45. */
#define NXTLL_REG_KP_VALUE                                                     \
  0x46 /*!< NXTLineLeader Kp value register. Default = 25. */
#define NXTLL_REG_KI_VALUE                                                     \
  0x47 /*!< NXTLineLeader Ki value register. Default = 0. */
#define NXTLL_REG_KD_VALUE                                                     \
  0x48 /*!< NXTLineLeader Kd value register. Default = 8. */
#define NXTLL_REG_CALIBRATED                                                   \
  0x49 /*!< NXTLineLeader calibrated sensor reading registers. 8 bytes. */
#define NXTLL_REG_WHITELIMITS                                                  \
  0x51 /*!< NXTLineLeader white limit registers. 8 bytes. */
#define NXTLL_REG_BLACKLIMITS                                                  \
  0x59 /*!< NXTLineLeader black limit registers. 8 bytes. */
#define NXTLL_REG_KP_FACTOR                                                    \
  0x61 /*!< NXTLineLeader Kp factor register. Default = 32. */
#define NXTLL_REG_KI_FACTOR                                                    \
  0x62 /*!< NXTLineLeader Ki factor register. Default = 32. */
#define NXTLL_REG_KD_FACTOR                                                    \
  0x63 /*!< NXTLineLeader Kd factor register. Default = 32. */
#define NXTLL_REG_WHITEDATA                                                    \
  0x64 /*!< NXTLineLeader white calibration data registers. 8 bytes. */
#define NXTLL_REG_BLACKDATA                                                    \
  0x6C /*!< NXTLineLeader black calibration data registers. 8 bytes. */
#define NXTLL_REG_RAWVOLTAGE                                                   \
  0x74    /*!< NXTLineLeader uncalibrated sensor voltage registers. 16 bytes. */
/** @} */ // end of NXTLineLeaderRegisters group

/** @defgroup NXTLineLeaderCommands MindSensors NXTLineLeader commands
 * NXTLineLeader device command constants. These are written to the command
 * register to control the device.
 * @{
 */
...
#define RFID_MODE_SINGLE                                                       \
  1 /*!< Configure the RFID device for a single reading */
#define RFID_MODE_CONTINUOUS                                                   \
  2       /*!< Configure the RFID device for continuous reading */
...
#define RICOpDescription(_Options, _Width, _Height)                            \
  8, 0, 0, 0, (_Options) & 0xFF, (_Options) >> 8, (_Width) & 0xFF,             \
      (_Width) >> 8, (_Height) & 0xFF, (_Height) >> 8

/**
 * Output an RIC CopyBits opcode
 * \param _CopyOptions CopyBits copy options.  See \ref
 * DisplayDrawOptionConstants.
 * \param _DataAddr The address of the sprite from which to copy data.
 * \param _SrcRect The rectangular portion of the sprite to copy.  See \ref
 * RICImgRect.
...
#define RICOpCopyBits(_CopyOptions, _DataAddr, _SrcRect, _DstPoint)            \
  18, 0, 3, 0, (_CopyOptions) & 0xFF, (_CopyOptions) >> 8, (_DataAddr) & 0xFF, \
      (_DataAddr) >> 8, _SrcRect, _DstPoint
...
#define RICOpPixel(_CopyOptions, _Point, _Value)                               \
  10, 0, 4, 0, (_CopyOptions) & 0xFF, (_CopyOptions) >> 8, _Point,             \
...
#define RICOpRect(_CopyOptions, _Point, _Width, _Height)                       \
  12, 0, 6, 0, (_CopyOptions) & 0xFF, (_CopyOptions) >> 8, _Point,             \
      (_Width) & 0xFF, (_Width) >> 8, (_Height) & 0xFF, (_Height) >> 8

/**
 * Output an RIC Circle opcode
 * \param _CopyOptions Circle copy options.  See \ref
 * DisplayDrawOptionConstants.
...
#define RICOpCircle(_CopyOptions, _Point, _Radius)                             \
  10, 0, 7, 0, (_CopyOptions) & 0xFF, (_CopyOptions) >> 8, _Point,             \
      (_Radius) & 0xFF, (_Radius) >> 8

/**
 * Output an RIC NumBox opcode
 * \param _CopyOptions NumBox copy options.  See \ref
 * DisplayDrawOptionConstants.
...
#define RICOpNumBox(_CopyOptions, _Point, _Value)                              \
  10, 0, 8, 0, (_CopyOptions) & 0xFF, (_CopyOptions) >> 8, _Point,             \
      (_Value) & 0xFF, (_Value) >> 8
...
#define RICOpVarMap(_DataAddr, _MapCount, _MapFunction)                        \
  ((_MapCount * 4) + 6) & 0xFF, ((_MapCount * 4) + 6) >> 8, 2, 0,              \
      (_DataAddr) & 0xFF, (_DataAddr) >> 8, (_MapCount) & 0xFF,                \
 ...
 */
#define RICMapElement(_Domain, _Range)                                         \
  (_Domain) & 0xFF, (_Domain) >> 8, (_Range) & 0xFF, (_Range) >> 8
 ...
 * \param _CopyOptions Polygon copy options.  See \ref
 * DisplayDrawOptionConstants.
...
#define RICOpPolygon(_CopyOptions, _Count, _ThePoints)                         \
  ((_Count * 4) + 6) & 0xFF, ((_Count * 4) + 6) >> 8, 10, 0,                   \
 ...
 * See \ref RICImgPoint.
 */
#define RICPolygonPoints(_pPoint1, _pPoint2, ...)                              \
  _pPoint1, _pPoint2, __VA_ARGS__

/**
 * Output an RIC Ellipse opcode
 * \param _CopyOptions Ellipse copy options.  See \ref
 * DisplayDrawOptionConstants.
...
#define RICOpEllipse(_CopyOptions, _Point, _RadiusX, _RadiusY)                 \
  12, 0, 9, 0, (_CopyOptions) & 0xFF, (_CopyOptions) >> 8, _Point,             \
      (_RadiusX) & 0xFF, (_RadiusX) >> 8, (_RadiusY) & 0xFF, (_RadiusY) >> 8
...
#define ULONG_MAX                                                              \
  4294967295 /*!< The maximum value of the unsigned long type                  \
              */
#define RAND_MAX                                                               \
  32768   /*!< The maximum unsigned int random number returned by rand */
